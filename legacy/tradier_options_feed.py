...
class TradierOptionsFeed:
    ...
    def __init__(self):
        ...
        if self._configured:
            ...
            logger.debug("Tradier options feed initialized (%s)", env)
        else:
            if not _REQUESTS_OK:
                logger.debug("Tradier: 'requests' library not installed")
            else:
                logger.debug("Tradier: TRADIER_API_TOKEN not set")
    ...
    def _get(self, path: str, params: dict = None) -> Optional[dict]:
        ...
        try:
            ...
            # Auth failure (401/403) means the token is invalid for this
            # endpoint — Tradier won't recover within this process, so
            # mark the feed unconfigured to short-circuit every subsequent
            # call instead of spamming the log.
            if resp.status_code in (401, 403):
                if self._configured:
                    logger.warning(
                        "Tradier returned %s for %s — auto-disabling feed "
                        "for the rest of this process (check TRADIER_API_TOKEN "
            ...
            logger.debug("Tradier API error (%s): %s", path, exc)
            return None
    ...
    def get_chain(self, ticker: str, expiry: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug("Tradier chain parse error (%s/%s): %s", ticker, expiry, exc)
            return None
    ...
    def stream_option_sweeps(self, tickers: List[str], callback) -> None:
        ...
        """
        if not self._configured:
            logger.debug("Tradier sweep streaming: not configured (no API token)")
            return
        # TODO: implement streaming worker when account is verified
        logger.debug("Tradier sweep streaming: stub ready — implement _stream_worker()")
