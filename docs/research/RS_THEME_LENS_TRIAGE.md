...
*Generated 2026-06-13T00:39:38.170659+00:00 · research-only · cache-only. Routing labels only — not buy/sell signals, not paper signals, not trade proposals. Does NOT modify the production universe, strategy gates, execution, or governance.*
...
| SATL | overlap | BREAKOUT_CONFIRMED | 48.0 | hardware | extended | Bearish but oversold | BLOCK | unusable | alpha_board_cap | real_quality | **BLOCKED** |
...
| OPTX | overlap | BREAKOUT_CONFIRMED | 47.0 | hardware | extended | Neutral | BLOCK | — | alpha_board_cap | real_quality | **BLOCKED** |
| VOYG | overlap | BREAKOUT_CONFIRMED | 43.9 | space_aerospace | extended | Neutral | BLOCK | poor | alpha_board_cap | gate_design_mismatch | **BLOCKED** |
...
| DIA | proposed_dynamic | EARLY_ACCUMULATION | 69.9 | other | near_ema20 | Bullish but not buyable yet | BLOCK | ok | alpha_board_cap | passes_a_gate | **NEEDS_GATEKEEPER** |
...
| RGTI | theme | EMERGING_MOMENTUM | 57.1 | hardware | constructive | Avoid / no edge | BLOCK | unusable | alpha_board_cap | real_quality | **BLOCKED** |
...
- **build/refresh Stock Lens (PROVIDER calls — operator approval required)** — ~22 stock-lens builds (Alpaca bars + FMP profile/options per ticker)
  ```
  ./scripts/run_research_cycle.sh lens OUST VPG ALAB VSH AMBQ ENPH VELO NVTS ATOM WOLF APLS XPO BIO PSN LUNR BTU LPTH INFQ MT JHX RKLB ILMN
  ```
- **refresh Executive Gatekeeper (cache-first; FMP earnings calendar only)** — ~3 gatekeeper rebuilds (cache-first, no per-ticker provider fan-out)
  ```
  ./scripts/run_research_cycle.sh gatekeeper-refresh --watch RKLB DIA SPOT
  ```
