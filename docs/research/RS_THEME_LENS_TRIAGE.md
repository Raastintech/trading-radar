...
*Generated 2026-06-19T00:35:05.358330+00:00 · research-only · cache-only. Routing labels only — not buy/sell signals, not paper signals, not trade proposals. Does NOT modify the production universe, strategy gates, execution, or governance.*
...
- Bucketed reasons: `{'no_atr_contraction': 26, 'too_extended': 17, 'volume_insufficient': 17, 'no_breakout': 12, 'insufficient_history_260': 9, 'unknown': 3, 'ma200_missing': 1}`
...
| VELO | overlap | LATE_EXTENDED | 38.3 | hardware | extended | — | — | — | alpha_board_cap | gate_design_mismatch | **TOO_EXTENDED** |
...
| LPTH | proposed_dynamic | PULLBACK_RECLAIM | 62.5 | hardware | near_ema20 | — | — | — | alpha_board_cap | gate_design_mismatch | **NEEDS_LENS** |
| ILMN | proposed_dynamic | PULLBACK_RECLAIM | 61.5 | biotech_healthcare | near_ema20 | Bullish but not buyable yet | WATCH | poor | alpha_board_cap | real_quality | **NEEDS_LENS** |
...
- **build/refresh Stock Lens (PROVIDER calls — operator approval required)** — ~27 stock-lens builds (Alpaca bars + FMP profile/options per ticker)
  ```
  ./scripts/run_research_cycle.sh lens ALAB OUST VELO VPG VSH ENPH NVTS ATOM WOLF AMBQ AMPX XPO APLS INFQ PSN LUNR BTU BURL LPTH OPTX SATL VOYG ASTS FLY RGTI ANET ILMN
  ```
